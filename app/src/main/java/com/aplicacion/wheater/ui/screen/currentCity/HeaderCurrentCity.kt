...
fun lineBarGraph() {
    ...
    Column(modifier = Modifier.padding(start = paddingStart, end = paddingEnd).fillMaxWidth()) {
        Text(text = "Temperature vs. humidity", style = textStyleH2, modifier = Modifier
            .align(Alignment.CenterHorizontally)
            .padding(top = 30.dp))
        Row(modifier = Modifier.padding(top = 50.dp).fillMaxWidth()) {
            Text(text = stringResource(id = R.string.current_city_switch_favorite), style = textStyleH3)
            Switch(modifier = Modifier.fillMaxWidth().wrapContentWidth(align = Alignment.End).padding(bottom = paddingEnd),
